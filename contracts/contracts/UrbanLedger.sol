...
contract UrbanLedger {
    ...
    event HashCommitted(string layerType, uint16 year, string sha256Hash, uint256 timestamp);
    ...
    function commitLayerHash(
        ...
        emit HashCommitted(_layerType, _year, _sha256Hash, block.timestamp);
    }

    /**
     * @dev Fetches the trusted hash for a specific layer and year to verify against the database.
     */
    function verifyLayer(string memory _layerType, uint16 _year) public view returns (string memory, uint256, string memory) {
        ...
        return (record.sha256Hash, record.timestamp, record.sourceRef);
    }
}
